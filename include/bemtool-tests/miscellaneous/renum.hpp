...
template<int dim>
std::vector<int> no_boundary_nodes(Dof< BasisFct<P1,dim>> dof){
...
template<int dim>
std::vector<int> boundary_nodes(Dof< BasisFct<P1,dim>> dof){
...
template<int dim>
std::vector<int> is_boundary_nodes(Dof< BasisFct<P1,dim>> dof){
    ...
    std::vector<int> boundary(nb_dofs,0);
    for (int i=0;i<boundary_edges.size();i++){
        boundary[boundary_edges[i].first]=1;
        boundary[boundary_edges[i].second]=1;
    }
