...
int main(int argc, char *argv[]) {
    ...
    std::vector<Cplx> sol_SL=pot_SL*sol;
    ...
    for (int i =0;i<nb_dof_output;i++){
        rad_phase[i]=atan2(std::imag(sol_vol[i]+uinc[i]),std::real(sol_vol[i]+uinc[i]));
        rad_real[i]=std::real(sol_vol[i]+uinc[i]);
        rad_abs[i]=std::abs(sol_vol[i]+uinc[i]);
    }
    ...
    pot_DL.print_infos();
    ddm.print_infos();
    CBIO.save_infos((outputpath+"infos_V.txt").c_str());
