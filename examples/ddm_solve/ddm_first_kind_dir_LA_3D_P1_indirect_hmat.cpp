...
int main(int argc, char *argv[]) {
   ...
   ////////////////////////////////////////////  Inputs
    ...
    BIO_Generator<LA_SL_3D_P1xP1,P1_2D> generator_V(dof,kappa);
    ...
    for (int i =0;i<nb_dof_output;i++){
        rad_phase[i]=atan2(std::imag(sol_SL[i]),std::real(sol_SL[i]));
        rad_real[i]=std::real(sol_SL[i]);
        rad_abs[i]=std::abs(sol_SL[i]);
    }
