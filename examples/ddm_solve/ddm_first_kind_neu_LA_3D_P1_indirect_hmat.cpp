...
int main(int argc, char *argv[]) {
    ...
    BIO_Generator<LA_HS_3D_P1xP1,P1_2D> generator_W(dof,kappa);
    ...
    for (int i =0;i<nb_dof_output;i++){
        rad_phase[i]=atan2(std::imag(sol_DL[i]),std::real(sol_DL[i]));
        rad_real[i]=std::real(sol_DL[i]);
        rad_abs[i]=std::abs(sol_DL[i]);
    }
