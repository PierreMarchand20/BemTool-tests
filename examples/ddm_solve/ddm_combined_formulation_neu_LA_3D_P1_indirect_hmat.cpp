...
int main(int argc, char *argv[]) {
    ...
    for (int i=0;i<nb_dof;i++){
  	...
  	// Operator
    Potential<PotKernel<LA,SL_POT,3,P1_2D>> POT_SL(mesh,kappa);
  	Potential<PotKernel<LA,DL_POT,3,P1_2D>> POT_DL(mesh,kappa);

    // Generator
    if (rank==0)
       std::cout << "Creating generators"<<std::endl;
    Combined_BIO_Generator<LA_HS_3D_P1xP1,LA_TDL_3D_P1xP1,P1_2D> generator_CBIO(dof,kappa,combined_coef,0.5);
    POT_Generator<PotKernel<LA,SL_POT,3,P1_2D>,P1_2D> generator_SL(POT_SL,dof,node_output);
    POT_Generator<PotKernel<LA,DL_POT,3,P1_2D>,P1_2D> generator_DL(POT_DL,dof,node_output);
    ...
    Partition(CBIO.get_MasterOffset_t(), CBIO.get_permt(),dof,cluster_to_ovr_subdomain,ovr_subdomain_to_global,neighbors,intersections);
    ...
    for (int i =0;i<nb_dof_output;i++){
        rad_phase[i]=atan2(std::imag(sol_vol[i]),std::real(sol_vol[i]));
        rad_real[i]=std::real(sol_vol[i]);
        rad_abs[i]=std::abs(sol_vol[i]);
    }
    ...
    DL.print_infos();
    ddm.print_infos();
    CBIO.save_infos((outputpath+"infos_W.txt").c_str());
