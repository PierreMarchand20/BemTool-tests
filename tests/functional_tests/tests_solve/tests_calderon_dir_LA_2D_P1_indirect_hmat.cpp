...
int main(int argc, char *argv[]) {
    ...
    for (int i=0;i<nb_elt;i++){
        const array<2,   int>& jdof = dof[i];
        mat<2,2,   Real>   M_local = MassP1(mesh[i]);

        for (int j=0;j<2;j++){
            for(int k=0;k<2;k++){
                M(jdof[j],jdof[k])=M(jdof[j],jdof[k])+M_local(j,k);
            }
        }
    }
    ...
    htool::Calderon<htool::partialACA,Cplx> calderon(V,W,M);
    calderon.solve(rhs.data(),sol.data());
