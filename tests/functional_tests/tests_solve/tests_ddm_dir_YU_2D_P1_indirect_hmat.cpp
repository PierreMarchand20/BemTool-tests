...
int main(int argc, char *argv[]) {
    ...
    for (int i=0;i<nb_dof;i++){
  	...
  	// Operator
  	Potential<PotKernel<YU,SL_POT,2,P1_1D>> POT_SL(mesh,kappa);

    // Generator
    SubBIO_Generator<YU_SL_2D_P1xP1,P1_1D> generator_V(dof,kappa);
    POT_Generator<PotKernel<YU,SL_POT,2,P1_1D>,P1_1D> generator_SL(POT_SL,dof,node_output);
