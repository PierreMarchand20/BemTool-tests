...
template <typename T>
class FFMatrix : public htool::IMatrix<T>{
    htool::Matrix<T> M;

public:
    FFMatrix(const std::string& name):htool::IMatrix<T>(1,1){
        ...
        std::cout << this->nr << " "<<this->nc << std::endl;
    }


    T get_coef(const int& i, const int& j) const {
      return M(i,j);
    }
