...
int main(int argc, char *argv[]) {
    ...
    for (int i=0;i<nb_dof;i++){
  	...
  	// Operator
  	Potential<PotKernel<YU,SL_POT,3,P1_2D>> POT_SL(mesh,kappa);

    // Generator
    SubBIO_Generator<YU_SL_3D_P1xP1,P1_2D> generator_V(dof,kappa);
    POT_Generator<PotKernel<YU,SL_POT,3,P1_2D>,P1_2D> generator_SL(POT_SL,dof,node_output);
    ...
    for (int i=0;i<nb_elt;i++){
        ...
        C3 Uinc;
        Uinc[0]= 1;//exp( -kappa*(xdof[0],dir) );
        Uinc[1]= 1;//exp( -kappa*(xdof[1],dir) );
        Uinc[2]= 1;//exp( -kappa*(xdof[1],dir) );
